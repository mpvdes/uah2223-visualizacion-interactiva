//http://jsfiddle.net/jagracar/fqduk0wv/
...
var sketch = function (p) {
    ...
    function paintNewTree() {
        var position = p.createVector(0.5 * p.width, 0.95 * p.height, 0);//posicion inicial del arbol
        var length = p.height / 7;//altura
        var diameter = length / 4.5;//diametro
        var angle = -p.HALF_PI + (p.PI / 180) * p.random(-5, 5); //angulo
        var color = p.color(130, 80, 20); //color
        var level = 1; 
